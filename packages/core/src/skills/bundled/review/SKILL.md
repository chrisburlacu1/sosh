...
1. `.qwen/review-rules.md` (Sosh native)
...
4. `SOSH.md` — extract only the `## Code Review` section if present
...
**Before posting**, check for existing Sosh review comments: `gh api repos/{owner}/{repo}/pulls/{pr_number}/comments --jq '.[] | select(.body | test("via Sosh /review")) | .id'`. If found, inform the user and let them decide whether to proceed.
  ...
  "comments": [
    {
      "path": "src/file.ts",
      "line": 42,
      "body": "**[Critical]** issue description\n\n```suggestion\nfix code\n```\n\n_— YOUR_MODEL_ID via Sosh /review_"
    }
  ]
}
````
...
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  ...
  -f event="APPROVE" \
  -f body="No issues found. LGTM! ✅ _— YOUR_MODEL_ID via Sosh /review_"
```
