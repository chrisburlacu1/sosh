...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for SOSH.md directly in the home directory
...
/**
 * Loads hierarchical SOSH.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length > 0) {
    ...
    // Only count files that match configured memory filenames (e.g., SOSH.md),
  ...
  if (!memoryContent && filePaths.length === 0 && ruleCount === 0) {
    logger.debug('No SOSH.md files or rules found.');
  }
