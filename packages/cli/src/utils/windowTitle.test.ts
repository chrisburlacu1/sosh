...
describe('computeWindowTitle', () => {
  ...
  it('should use default Sosh title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Sosh - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Sosh - project-name_v1.0');
  });
});
