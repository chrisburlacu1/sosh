...
export async function checkForUpdates(): Promise<UpdateObject | null> {
  try {
    return null; // Disabled to prevent external network traffic
    ...
    //     const message = `A new version of Sosh is available! ${currentVersion} → ${bestUpdate.latest}`;
    //     return {
    //       message,
    ...
    //     const message = `Sosh update available! ${currentVersion} → ${updateInfo.latest}`;
    //     return {
    //       message,
